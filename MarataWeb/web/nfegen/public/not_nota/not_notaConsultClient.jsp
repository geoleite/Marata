...
<jsp:useBean id="not_notaJB" class="br.com.easynet.nfegen.jb.Not_notaConsultClientJB" scope="request"/>                                         
...
<form name="not_nota" action="portal.jsp" method="post">                                         
    ...
    <input type="hidden" name="urlCanal" value="/nfegen/public/not_nota/not_notaConsultClient.jsp"/>
    <div align="center">     
        ...
        <table>
            ...
            <tr>
                <td>
                    <label title="Emrpesas Associadas" >
                        Emp. Assoc.:
                    </label>
                </td>
                <td>
                    <select name="emp_empresaT.emp_nr_id" onchange="setOpPortal('not_nota',' ', '/nfegen/public/not_nota/not_notaConsultClient.jsp')">
                        <!--option value="0" ${0==not_notJB.emp_empresaT.emp_nr_id?'selected':' '}>Todas as Empresas</option-->
                        <c:forEach var="item" items="${not_notaJB.listEmp}">
                                    <option  ${not_notaJB.emp_empresaT.emp_nr_id eq item.emp_nr_id?'selected':' '} value="${item.emp_nr_id}">${item.emp_tx_nome} - ${item.emp_tx_cnpj}</option>
                        </c:forEach>
                    </select>
                </td>
            </tr>
            ...
            <tr>
                <td colspan="2" align="center">
                    ...
                    <input type="button" value="Pesquisar" onClick="setOpPortal('not_nota','consult', '/nfegen/public/not_nota/not_notaConsultClient.jsp')"/>&nbsp;&nbsp;&nbsp;
        ...
        <display:table class="tabela" cellpadding="0" cellspacing="0"   id="item" name="${not_notaJB.list}"  pagesize="${not_notaJB.tamanhoConsulta}" >
            ...
            <display:column sortable="false" title="Tipo" >
                <label>
                    <c:out value="${item.not_tx_tipo=='E'?'Sainda':'Entrada'}"/>
                </label>
            </display:column>
